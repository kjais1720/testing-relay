...
export const defaultRolesToExclude = (companies: Company[]): string[] => {
    ...
    const locationCount = companies!.reduce(
        (a, b) => a + (b.locations ? b.locations!.edges!.length : 0),
        0
    )
    if (locationCount > 1) {
...
const filterRoles = (roles: DesignationMaster_designations['roles']['role'], exclude: string[]) =>
    roles.filter(
        (r) => (r && !r.isDefault) || (r.isDefault && !exclude.find((k) => r.roleName.includes(k)))
    )
const filterDesignations = (
    designations: DesignationMaster_designations['designations']['designation'],
    exclude: string[]
) => {
...
const DesignationMaster: React.FC<Props> = ({ layoutProps, designations: _, parent }) => {
    ...
    const canManageEditor = (id: string) =>
        can([Roles.DesignationsAdmin, Roles.DesignationsEditor], id)

    const header = <Trans>Designations</Trans>
    const subHeader = (
        <Trans>Designation is an official role or title of a person in your organization</Trans>
    )
    ...
    const roles = _.roles?.role ?? []
    const designation = _.designations?.designation ?? []
    const allRoles = roles
        .filter(Boolean)
        .filter((r) => r && ['com', 'grp', 'loc', 'emp'].includes(r.level))
    const exclude = defaultRolesToExclude(companies!)
    const rolesArr = canManageGroup
        ? filterRoles(allRoles, exclude)
        : filterRoles(allRoles, [...exclude, 'Owner', 'Co-Owner'])
    const designations = filterDesignations(designation, exclude)

    const col1 = !designations.length ? (
        <DesignationEmptyState
            canManage={canManageAdmin(parent)}
        ...
        <DesignationList
            ...
            onItemClick={
                canManageEditor(parent)
                    ? (id: string) => navigate(`${window.btoa(id!)}/update`)
                    : undefined
            }
        />
    )
            ...
            <Routes>
                {canManageAdmin(parent) && (
                    <>
                        <Route
                            path="add"
                            element={<DesignationAdd roles={rolesArr} variables={variables} />}
                        />
                        <Route
                            path=":id/delete"
                            element={<DesignationDelete variables={variables} />}
                        />
                    </>
                )}
                {canManageEditor(parent) && (
...
export default createFragmentContainer(DesignationMaster, {
    designations: graphql`
        fragment DesignationMaster_designations on Query
